...
void task_remove(struct TASK *task){
    ...
    tl->running--;
    if (i < tl->now) tl->now--; //ズレをなくす
    if (tl->now >= tl->running) tl->now = 0; //nowを修正
    task->flags = 1; //動作していない状態
    for (; i < tl->running; i++) { //ずらし
...
void task_switchsub(void){
    int i;
    for (i = 0; i < MAX_TASKLEVELS; i++){ //一番上を探す
...
void task_run(struct TASK *task, int level, int priority){
    ...
    taskctl->lv_change = 1; //次回タスクスイッチの際にレベルを見直す
    return;
}
