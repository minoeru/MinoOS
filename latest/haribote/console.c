...
void console_task(struct SHEET *sheet, int memtotal){
  ...
  for (;;){
    ...
    if (fifo32_status(&task->fifo) == 0){
      ...
      if (i == 2) cons.cur_c = COL8_FFFFFF; //カーソルon
      if (i == 3) { //カーソルoff
        if (cons.sht != 0) boxfill8(cons.sht->buf, cons.sht->bxsize, COL8_000000, cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
		cons.cur_c = -1;
      }
	  if (i == 4) cmd_exit(&cons, fat);
      if (256 <= i && i <= 511){ //キーボード入力
        if (i == 8 + 256){ //バックスペース
          if (cons.cur_x > 16){
            cons_putchar(&cons, ' ', 0);
			cons.cur_x -= 8;
      ...
      //カーソル再表示
	  if (cons.sht != 0){
		  if (cons.cur_c >= 0) boxfill8(cons.sht->buf, cons.sht->bxsize, cons.cur_c, cons.cur_x, cons.cur_y, cons.cur_x + 7, cons.cur_y + 15);
...
void cons_newline(struct CONSOLE *cons){
	int x, y;
  	...
  	struct TASK *task = task_now();
	if (cons->cur_y < 28 + 112) cons->cur_y += 16; // 次の行へ
	else {
		if (sheet != 0){
			for (y = 28; y < 28 + 112; y++) {
...
void cons_runcmd(char *cmdline, struct CONSOLE *cons, int *fat, int memtotal){
	if (strcmp(cmdline, "mem") == 0 && cons->sht != 0) cmd_mem(cons, memtotal);
	else if (strcmp(cmdline, "clear") == 0 && cons->sht != 0) cmd_clear(cons);
	else if (strcmp(cmdline, "ls") == 0 && cons->sht != 0) cmd_ls(cons);
	// else if (strncmp(cmdline, "cat ", 4) == 0 && cons->sht != 0) cmd_cat(cons, fat, cmdline);
...
// 	} else {
// 		cons_putstr0(cons, "No such file or directory\n");
// 	}
// 	cons_newline(cons);
// 	return;
// }
...
int cmd_app(struct CONSOLE *cons, int *fat, char *cmdline){
	...
	name[i] = 0; // ファイルの後ろを0にする

	// ファイルを探す
	finfo = file_search(name, (struct FILEINFO *) (ADR_DISKIMG + 0x002600), 224);
	if (finfo == 0 && name[i - 1] != '.') {
		//見つからなければ後ろに.HRBをつけて探す
		name[i    ] = '.';
		name[i + 1] = 'H';
		name[i + 2] = 'R';
		name[i + 3] = 'B';
	...
	if (finfo != 0) { //ファイル発見
		...
		if (finfo->size >= 36 && strncmp(p + 4, "Hari", 4) == 0 && *p == 0x00){
			...
			for (i = 0; i < MAX_SHEETS; i++){
				...
				if ((sht->flags & 0x11) == 0x11 && sht->task == task) sheet_free(sht); //開きっぱなしを閉じる
			}
			for (i = 0; i < 8; i++) {
...
int *hrb_api(int edi, int esi, int ebp, int esp, int ebx, int edx, int ecx, int eax){
	...
	else if (edx == 14) sheet_free((struct SHEET *) ebx);
	else if (edx = 15){
		for (;;){
			io_cli();
			if (fifo32_status(&task->fifo) == 0){
				if (eax != 0) task_sleep(task); //fifoがない
				else{
					io_sti();
					reg[7] = -1;
					return 0;
	...
	else if (edx == 17) timer_init((struct TIMER *) ebx, &task->fifo, eax + 256);
	else if (edx == 18) timer_settime((struct TIMER *) ebx, eax);
	else if (edx == 19) timer_free((struct TIMER *) ebx);
	else if (edx == 20){
		if (eax == 0) {
			i = io_in8(0x61);
			io_out8(0x61, i & 0x0d);
	...
	else if (edx == 27) reg[7] = task->langmode;
	return 0;
}
