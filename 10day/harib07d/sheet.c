...
void sheet_refresh(struct SHTCTL *ctl, struct SHEET *sht, int bx0, int by0, int bx1, int by1){
    if (sht->height >= 0) sheet_refreshsub(ctl, sht->vx0 + bx0, sht->vy0 + by0, sht->vx0 + bx1, sht->vy0 + by1); //新しい下敷きの情報で書き直す
	return;
}
...
void sheet_free(struct SHTCTL *ctl, struct SHEET *sht){
    if (sht->height >= 0) sheet_updown(ctl, sht, -1); //表示中なら非表示にする
    sht->flags = 0; //未使用マーク
    return;
}