...
int strtol(char *s, char **endp, int base);	//後で作る
	...
	for (;;) {
		if (('A' <= *p && *p <= 'G') || *p == 'R') {
			...
			if (i != note_old) {
				...
				if (s[0] != 0) api_putstrwin(win + 1, 32, 36, 10, 4, s);
				api_refreshwin(win, 32, 36, 64, 52);
				if (28 <= note_old && note_old <= 107) api_boxfilwin(win, (note_old - 28) * 3 + 8, 60, (note_old - 28) * 3 + 10,  76, 7);
				if (28 <= i && i <= 107) api_boxfilwin(win, (i - 28) * 3 + 8, 60, (i - 28) * 3 + 10,  76, 4);
			...
			if (p[1] == 'K') {	
				...
				s[i] = 0;
				if (i != 0) api_putstrwin(win + 1, 128 - i * 4, 88, 0, i, s);
