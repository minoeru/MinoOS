...
void HariMain(void){
  ...
  *((int *) 0x0fe4) = (int) shtctl;
  ...
  nihongo = (unsigned char *) memman_alloc_4k(memman, 16 * 256 + 32 * 94 * 47);
	...
	if (finfo != 0) file_loadfile(finfo->clustno, finfo->size, nihongo, fat, (char *) (ADR_DISKIMG + 0x003e00));
	else {
		for (i = 0; i < 16 * 256; i++) {
			nihongo[i] = hankaku[i];
		}
  ...
  for(;;){
    ...
    if (fifo32_status(&fifo) == 0) {
      ...
      if (256 <= i && i <= 511){ //キーボードデータ
        ...
        if (s[0] != 0 && key_win != 0){ //通常文字
          fifo32_put(&key_win->task->fifo, s[0] + 256); //コンソールへ
        }
        ...
        if (i == 256 + 0x58) sheet_updown(shtctl->sheets[1], shtctl->top - 1); //F12
      ...
      else if (768 <= i && i <= 1023) close_console(shtctl->sheets0 + (i - 768));
      else if (1024 <= i && i <= 2023) close_console(shtctl->sheets0 + (i - 1024));
